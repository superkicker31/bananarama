Vue.component('bananarama-episode', {
  ...
  template: '<div class="flex flex-col episode-header md:rounded-r"><span class="lg:h-64 bg-red-100 text-gray-200 md:text-3xl text-2xl episode-bg-overlay flex items-end p-4 md:rounded-r rounded-b">{{ title }}</span><audio class="audio-player" v-bind:src="filename" preload="none" /></div>'
});
